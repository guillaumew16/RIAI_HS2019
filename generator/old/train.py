...
for epoch in range(1,args.num_epochs+1):
    ...
    for batch_idx, (x_batch, y_batch) in enumerate(tqdm(train_loader)):
        if args.defense == 'True':
            # do PGD attack to generate adversary data set
            model.eval() # switch to eval mode for faster PGD attack
            x_batch, y_batch = x_batch.to(args.device), y_batch.to(args.device)
            x_batch = pgd_untargeted_batched(model, x_batch, y_batch, 5, 0.08, 0.05, device=args.device)
